...
def test_all_products_200(client, admin_token_header):
    response = client.get('/admin/get_items', headers=admin_token_header)
...
def test_add_product_201(client, admin_token_header):
    ...
    response = client.post('/admin/create_items', json.dumps(data), headers=admin_token_header)
...
def test_update_product_not_found_404(client, admin_token_header):
    ...
    response = client.put('/update_item/100', json.dumps(data), headers=admin_token_header)
...
def test_delete_product_not_found_404(client, admin_token_header):
    response = client.delete('/admin/delete_item/100', headers=admin_token_header)
    assert response.status_code == 404
    assert "Item Not Found"
...
def test_view_orders_200(client, admin_token_header):
    response = client.get('/admin/view_orders', headers=admin_token_header)
    assert response.status_code == 200
    assert "Items Fetched Successfully"
...
#     }]
#     response = client.post('/admin/add_discount_coupon', json.dumps(data), headers=admin_token_header)
#     assert response.status_code == 200
...
def test_show_discount_coupon_200(client, admin_token_header):
    response = client.get('/admin/show_discount_coupon', headers=admin_token_header)
    assert response.status_code == 200
